...
// Side returns inside this envelope are treated as nearby objects rather than
// ordinary corridor geometry.  The Pi publishes a hard stop at 22 cm; the
// bridge still suppresses/redirects a nudge before that stop is received.
#define SIDE_NUDGE_AVOID_CM              40.0f
#define SIDE_NUDGE_HARD_STOP_CM          22.0f
...
static void computeNudgeCommand(const String& sidesVal,
  ...
  // ── Side-object avoidance interlock ────────────────────────
  // A person, bin, or fire extinguisher is a protrusion in the side return;
  // it must not be handled as a normal wall-centering error.  Prefer the
  // clear side when only one side is inside the avoidance envelope.  When
  // both sides are constrained, hold the heading and let the path STOP
  // packet / local ultrasonic protection take priority.
  const bool leftSideThreat = left <= SIDE_NUDGE_AVOID_CM;
  const bool rightSideThreat = right <= SIDE_NUDGE_AVOID_CM;
  if (leftSideThreat && !rightSideThreat) {
    wantDir = -1;  // move away from the left-side object
  } else if (rightSideThreat && !leftSideThreat) {
    wantDir = 1;   // move away from the right-side object
  } else if (leftSideThreat && rightSideThreat) {
    wantDir = 0;   // no safe lateral correction
  }

  // At the hard side envelope, never issue a lateral tap.  The Pi's matching
  // side-stop packet should already latch the controller, but this protects
  // the bridge if a status packet is delayed or a legacy sender is used.
  if (left <= SIDE_NUDGE_HARD_STOP_CM || right <= SIDE_NUDGE_HARD_STOP_CM) {
    wantDir = 0;
  }
