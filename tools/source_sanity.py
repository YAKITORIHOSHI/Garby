...
def main() -> int:
    failures = []
    IGNORE_DIRS = {".git", ".gradle", "build", ".venv", ".mcu_flasher_build_cache", "node_modules", ".idea", ".externalNativeBuild", ".cxx"}

    def is_ignored(p: Path) -> bool:
        return any(part in IGNORE_DIRS for part in p.parts)

    cpp_files = sorted(p for p in ROOT.rglob("*") if p.is_file() and p.suffix.lower() in CPP_EXT and not is_ignored(p))
    json_files = sorted(p for p in ROOT.rglob("*.json") if p.is_file() and not is_ignored(p))
