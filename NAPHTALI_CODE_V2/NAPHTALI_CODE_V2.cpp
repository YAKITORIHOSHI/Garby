...
void haltAndWait(const String& reason) {
  ...
  const bool requiresLocal = frontObstacleDetected() ||
                             reason.indexOf("HUMAN") >= 0 ||
...
bool frontObstacleDetected() {
  ...
  // A fresh single echo inside the stop envelope also stops immediately; the
  // confirmation latch remains responsible for persistence and recovery.
  return frontObstacleLatched ||
         (frontDistanceFresh() &&
          latestFrontDistance() <= ULTRASONIC_STOP_DISTANCE_CM);
}
...
bool moveToTarget(long target1, long target2, bool isADJ, bool isNudgeEnabled) {
  ...
  while (motorsRunning()) {
    ...
    // The centered Servo+Ultrasonic has priority over steering. Do this
    // before consuming a nudge command so a fresh close echo cannot receive
    // even a one-loop lateral correction.
    if (frontObstacleDetected()) {
      activeBrakeStopMotors();
      xTaskCreate(buzzerTask, "garby-obstacle", 1536, nullptr, 2, nullptr);
      haltAndWait("SONIC:BLOCKED before nudge");
      if (resetQueued || shouldStop || !pathCommandFresh()) return false;
      if (!startTargetMove(target1, target2)) return false;
      commandServoTracked(DEFAULT_VIEW);
      continue;
